  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_cdt_summary_mob" == sTestField)
        {
...
?>
  ...
  // ---------- Validate id
  function do_ajax_cdt_summary_mob_validate_id()
  {
    var nomeCampo_id = "id";
    var var_id = scAjaxGetFieldHidden(nomeCampo_id);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_cdt_summary_mob_validate_id(var_id, var_script_case_init, do_ajax_cdt_summary_mob_validate_id_cb);
  } // do_ajax_cdt_summary_mob_validate_id

  function do_ajax_cdt_summary_mob_validate_id_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_cdt_summary_mob_validate_id_cb

  // ---------- Validate aula
  function do_ajax_cdt_summary_mob_validate_aula()
  {
    var nomeCampo_aula = "aula";
    var var_aula = scAjaxGetFieldSelect(nomeCampo_aula);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_cdt_summary_mob_validate_aula(var_aula, var_script_case_init, do_ajax_cdt_summary_mob_validate_aula_cb);
  } // do_ajax_cdt_summary_mob_validate_aula

  function do_ajax_cdt_summary_mob_validate_aula_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "aula";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_cdt_summary_mob_validate_aula_cb

  // ---------- Validate conteudo
  function do_ajax_cdt_summary_mob_validate_conteudo()
  {
    var nomeCampo_conteudo = "conteudo";
    var var_conteudo = scAjaxGetFieldEditorHtml(nomeCampo_conteudo);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_cdt_summary_mob_validate_conteudo(var_conteudo, var_script_case_init, do_ajax_cdt_summary_mob_validate_conteudo_cb);
  } // do_ajax_cdt_summary_mob_validate_conteudo

  function do_ajax_cdt_summary_mob_validate_conteudo_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "conteudo";
    scEventControl_onBlur(sFieldValid);
    scAjaxUpdateFieldErrors(sFieldValid, "valid");
    sFieldErrors = scAjaxListFieldErrors(sFieldValid, false);
    if ("" == sFieldErrors)
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_cdt_summary_mob_validate_conteudo_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_cdt_summary_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_cdt_summary_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_id = scAjaxGetFieldHidden("id");
    var var_aula = scAjaxGetFieldSelect("aula");
    var var_conteudo = scAjaxGetFieldEditorHtml("conteudo");
    ...
    scAjaxProcOn();
    x_ajax_cdt_summary_mob_submit_form(var_id, var_aula, var_conteudo, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_cdt_summary_mob_submit_form_cb);
  } // do_ajax_cdt_summary_mob_submit_form

  function do_ajax_cdt_summary_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    ...
    if (scAjaxIsOk())
    {
      ...
      scAjaxHideErrorDisplay("aula");
      scAjaxHideErrorDisplay("conteudo");
      scLigEditLookupCall();
<?php
...
?>
    ...
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_cdt_summary_mob_submit_form_cb_after_alert);
  } // do_ajax_cdt_summary_mob_submit_form_cb
  function do_ajax_cdt_summary_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_cdt_summary_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_cdt_summary_mob_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_cdt_summary_mob_navigate_form, function() {});
    } else {
      perform_ajax_cdt_summary_mob_navigate_form();
    }
  }

  function perform_ajax_cdt_summary_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    scAjaxProcOn();
    x_ajax_cdt_summary_mob_navigate_form(var_id, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search,  var_nmgp_cond_fast_search,  var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_cdt_summary_mob_navigate_form_cb);
  } // do_ajax_cdt_summary_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['cdt_summary_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_cdt_summary_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_cdt_summary_mob_navigate_form_cb_after_alert);
  } // do_ajax_cdt_summary_mob_navigate_form_cb
  function do_ajax_cdt_summary_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
...
{
?>
    do_ajax_cdt_summary_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_cdt_summary_mob_navigate_form_cb_after_alert

  function sc_hide_cdt_summary_mob_form()
  {
  ...
  ajax_field_list[1] = "aula";
  ajax_field_list[2] = "conteudo";
  ...
  var ajax_error_list = {
    ...
    "aula": {"label": "Aula", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0},
    "conteudo": {"label": "Conteudo", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0}
  };
  ...
  ajax_field_mult["aula"][1] = "aula";
  ajax_field_mult["conteudo"][1] = "conteudo";

  var ajax_field_id = {
    "aula": new Array("hidden_field_label_aula", "hidden_field_data_aula"),
    "conteudo": new Array("hidden_field_label_conteudo", "hidden_field_data_conteudo")
  };
