...
$old_error_handler = set_error_handler("cdt_summary_scriptcase_error_handler");

function cdt_summary_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['cdt_summary']['contr_erro']) && $_SESSION['scriptcase']['cdt_summary']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_cdt_summary->contr_cdt_summary->Apl_com_erro = true;
            if ($inicial_cdt_summary->contr_cdt_summary->NM_ajax_flag)
            {
                $inicial_cdt_summary->contr_cdt_summary->NM_ajax_info['errList']['geral_cdt_summary'][] = $msg;
                return;
            }
            cdt_summary_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function cdt_summary_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['cdt_summary'])  && '' != $_SESSION['scriptcase']['error_icon']['cdt_summary'])  ? $_SESSION['scriptcase']['error_icon']['cdt_summary']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['cdt_summary']) && '' != $_SESSION['scriptcase']['error_close']['cdt_summary']) ? $_SESSION['scriptcase']['error_close']['cdt_summary'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['cdt_summary']) && '' != $_SESSION['scriptcase']['error_span_title']['cdt_summary']) ? $_SESSION['scriptcase']['error_span_title']['cdt_summary'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['cdt_summary']) && '' != $_SESSION['scriptcase']['error_icon_title']['cdt_summary']) ? $_SESSION['scriptcase']['error_icon_title']['cdt_summary'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['cdt_summary']['error_buffer'] .= ob_get_flush();
}
