...
$old_error_handler = set_error_handler("sub_studentCourse_scriptcase_error_handler");

function sub_studentCourse_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['sub_studentCourse']['contr_erro']) && $_SESSION['scriptcase']['sub_studentCourse']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sub_studentCourse->contr_sub_studentCourse->Apl_com_erro = true;
            if ($inicial_sub_studentCourse->contr_sub_studentCourse->NM_ajax_flag)
            {
                $inicial_sub_studentCourse->contr_sub_studentCourse->NM_ajax_info['errList']['geral_sub_studentCourse'][] = $msg;
                return;
            }
            sub_studentCourse_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sub_studentCourse_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sub_studentCourse'])  && '' != $_SESSION['scriptcase']['error_icon']['sub_studentCourse'])  ? $_SESSION['scriptcase']['error_icon']['sub_studentCourse']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sub_studentCourse']) && '' != $_SESSION['scriptcase']['error_close']['sub_studentCourse']) ? $_SESSION['scriptcase']['error_close']['sub_studentCourse'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sub_studentCourse']) && '' != $_SESSION['scriptcase']['error_span_title']['sub_studentCourse']) ? $_SESSION['scriptcase']['error_span_title']['sub_studentCourse'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sub_studentCourse']) && '' != $_SESSION['scriptcase']['error_icon_title']['sub_studentCourse']) ? $_SESSION['scriptcase']['error_icon_title']['sub_studentCourse'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['sub_studentCourse']['error_buffer'] .= ob_get_flush();
}
