  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_sub_studentCourse" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_sub_studentCourse_form();
	}
...
?>
  ...
  // ---------- Validate course_
  function do_ajax_sub_studentCourse_validate_course_(iNumLinha)
  {
    var nomeCampo_course_ = "course_" + iNumLinha;
    var var_course_ = scAjaxGetFieldSelect(nomeCampo_course_);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_sub_studentCourse_validate_course_(var_course_, iNumLinha, var_script_case_init, do_ajax_sub_studentCourse_validate_course__cb);
  } // do_ajax_sub_studentCourse_validate_course_

  function do_ajax_sub_studentCourse_validate_course__cb(sResp)
  {
    ...
    if ("" != iLineNumber)
    {
      sFieldValid = "course_" + iLineNumber;
    }
    else
    {
      sFieldValid = "course_";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_sub_studentCourse_validate_course__cb

  var sc_num_ult_line = "";
  var sc_insert_open  = false;

  // ---------- add_new_line
  function do_ajax_sub_studentCourse_add_new_line(sc_clone, sc_seq_clone)
  {
    if (sc_insert_open)
    {
        if (sc_clone == 'S' && sc_seq_clone != iAjaxNewLine)
        {
          do_ajax_sub_studentCourse_cancel_insert(iAjaxNewLine);
        }
        else
        {
          return;
    ...
    var var_script_case_init = document.F1.script_case_init.value;
    scAjaxProcOn(true);
    x_ajax_sub_studentCourse_add_new_line(var_sc_clone, var_sc_seq_clone, var_sc_seq_vert, var_script_case_init, do_ajax_sub_studentCourse_add_new_line_cb);
  } // do_ajax_sub_studentCourse_add_new_line

  function do_ajax_sub_studentCourse_add_new_line_cb(sResp)
  {
    scAjaxProcOff(true);
    if ("{" == sResp.substr(0, 1)) {
        oResp = scAjaxResponse(sResp);
    ...
    scFixCol_loadState();
  } // do_ajax_sub_studentCourse_add_new_line_cb

  // ---------- backup_line
  function do_ajax_sub_studentCourse_backup_line(iNumLinha)
  {
    var var_id_ = scAjaxGetFieldText("id_" + iNumLinha);
    var var_nmgp_refresh_row = iNumLinha;
    var var_nm_form_submit = document.F1.nm_form_submit.value;
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_sub_studentCourse_backup_line(var_id_, var_nmgp_refresh_row, var_nm_form_submit, var_script_case_init, do_ajax_sub_studentCourse_backup_line_cb);
  } // do_ajax_sub_studentCourse_backup_line

  function do_ajax_sub_studentCourse_backup_line_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    if (!scAjaxHasError())
  ...
  } // do_ajax_sub_studentCourse_backup_line_cb

  function do_ajax_sub_studentCourse_cancel_insert(iSeqVert)
  {
    var oTBodyOld = document.getElementById("hidden_bloco_0").tBodies[0];
    ...
    scAjaxHideErrorDisplay("table");
  } // do_ajax_sub_studentCourse_cancel_insert

  function do_ajax_sub_studentCourse_cancel_update(iSeqVert)
  {
    do_ajax_sub_studentCourse_backup_line(iSeqVert);
...
?>
  } // do_ajax_sub_studentCourse_cancel_update

  function do_ajax_sub_studentCourse_restore_buttons()
  {
<?php
...
?>
  } // do_ajax_sub_studentCourse_restore_buttons

  // ---------- table_refresh
  function do_ajax_sub_studentCourse_table_refresh()
  {
    ...
    scAjaxProcOn();
    x_ajax_sub_studentCourse_table_refresh(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_fast_search, var_nmgp_cond_fast_search, var_nmgp_arg_fast_search, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_sub_studentCourse_table_refresh_cb);
  } //  do_ajax_sub_studentCourse_table_refresh

  function do_ajax_sub_studentCourse_table_refresh_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxAlert(do_ajax_sub_studentCourse_table_refresh_cb_after_alert);
  } // do_ajax_sub_studentCourse_table_refresh_cb
  function do_ajax_sub_studentCourse_table_refresh_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    ...
    sc_insert_open = false;
  } // do_ajax_sub_studentCourse_table_refresh_cb_after_alert
...
function scJs_sweetalert_params(params) {
  ...
  var sc_num_ult_opc  = "";
  var sc_num_ult_tr   = "";
  function do_ajax_sub_studentCourse_submit_form(iNumLinha, indexTR)
  {
    if (scEventControl_active(iNumLinha)) {
      setTimeout(function() { do_ajax_sub_studentCourse_submit_form(iNumLinha, indexTR); }, 500);
      return;
    }
    sc_num_ult_line = iNumLinha;
    sc_num_ult_tr   = indexTR;
    scAjaxHideMessage();
    var var_course_ = scAjaxGetFieldSelect("course_" + iNumLinha);
    var var_nm_form_submit = document.F1.nm_form_submit.value;
...
?>
    x_ajax_sub_studentCourse_submit_form(var_course_, var_nmgp_refresh_row, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_sub_studentCourse_submit_form_cb);
  } // do_ajax_sub_studentCourse_submit_form

  function do_ajax_sub_studentCourse_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    scAjaxSetLabel(true);
    scAjaxSetMaster();
    scAjaxAlert(do_ajax_sub_studentCourse_submit_form_cb_after_alert);
  } // do_ajax_sub_studentCourse_submit_form_cb
  function do_ajax_sub_studentCourse_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_sub_studentCourse_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_sub_studentCourse_navigate_form()
  {
    if (scFormHasChanged) {
      scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', perform_ajax_sub_studentCourse_navigate_form, function() {});
    } else {
      perform_ajax_sub_studentCourse_navigate_form();
    }
  }

  function perform_ajax_sub_studentCourse_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    ...
    if (sc_insert_open)
    {
        do_ajax_sub_studentCourse_cancel_insert(sc_sub_studentCourse_get_last_line_number());
    }
    nm_uncheck_delete();
    scAjaxHideMessage();
    scAjaxHideErrorDisplay("table");
    ...
    var var_script_case_init = document.F2.script_case_init.value;
    scAjaxProcOn();
    x_ajax_sub_studentCourse_navigate_form(var_id_, var_nm_form_submit, var_nmgp_opcao, var_nmgp_ordem, var_nmgp_arg_dyn_search, var_script_case_init, do_ajax_sub_studentCourse_navigate_form_cb);
  } // do_ajax_sub_studentCourse_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['sub_studentCourse']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_sub_studentCourse_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scErrorLineReset();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_sub_studentCourse_navigate_form_cb_after_alert);
  } // do_ajax_sub_studentCourse_navigate_form_cb
  function do_ajax_sub_studentCourse_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
?>
  ...
  } // do_ajax_sub_studentCourse_navigate_form_cb_after_alert

    function sc_sub_studentCourse_get_last_line_number()
    {
    ...
    } //sc_sub_studentCourse_get_last_line_number

  function sc_hide_sub_studentCourse_form()
  {
...
?>
  ...
  {
    ajax_field_list[iTotCampos] = "course_" + iNumLinha;
    iTotCampos++;
    ajax_error_list["course_" + iNumLinha] = {"label": "Cursada<?php echo $sLineInfo; ?>", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 0};
    ajax_field_mult["course_"][iNumLinha] = "course_" + iNumLinha;
    ajax_field_id["course_" + iNumLinha] = new Array("hidden_field_label_course_", "hidden_field_data_course_" + iNumLinha);
    ajax_error_count["course_" + iNumLinha] = "off";
<?php
if (!$this->Grid_editavel)
{
?>
    ajax_read_only["course_" + iNumLinha] = "off";
<?php
}
else
{
?>
    ajax_read_only["course_" + iNumLinha] = "on";
<?php
}
?>
  }
  ...
  {
    ajax_error_list["course_" + iNumLinha] = null;
  }
  ...
  function scRemoveErrors()
  {
    for (iNumLinha = 1; iNumLinha < Lim_linhas; iNumLinha++)
    {
      ajax_error_list["course_" + iNumLinha]["valid"] = new Array();
      ajax_error_list["course_" + iNumLinha]["onblur"] = new Array();
      ajax_error_list["course_" + iNumLinha]["onchange"] = new Array();
      ajax_error_list["course_" + iNumLinha]["onclick"] = new Array();
      ajax_error_list["course_" + iNumLinha]["onfocus"] = new Array();
    }
  }
...
?>
  ...
  {
<?php
  $NM_contr_readonly = (isset($this->nmgp_cmp_readonly['course_'])) ? $this->nmgp_cmp_readonly['course_'] : 'off';
?>
    scAjaxFieldRead("course_" + iSeq, "<?php echo $NM_contr_readonly ?>");
  }

  function mdCloseObjects(iSeq)
  {
    scAjaxFieldRead("course_" + iSeq, "on");
    rerunHeaderDisplay = 1;
    scSetFixedHeaders(true);
  }
...
?>
  ...
  function scErrorLineOff(iRow, sIdError)
  {
    ...
    if (bErrorRow || ("off" == ajax_error_count["course_" + iRow]))
    {
      if (bErrorRow)
      {
        ajax_error_count["course_" + iRow] = "off";
      }
      ...
      $("#hidden_field_data_course_" + iRow).removeClass("scFormErrorLine");
    }
  }
