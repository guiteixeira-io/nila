...
function scEventControl_init(iSeqRow) {
  scEventControl_data["course_" + iSeqRow] = {"blur": false, "change": false, "autocomp": false, "original": "", "calculated": ""};
}

function scEventControl_active(iSeqRow) {
  if (scEventControl_data["course_" + iSeqRow] && scEventControl_data["course_" + iSeqRow]["blur"]) {
    return true;
  }
  if (scEventControl_data["course_" + iSeqRow] && scEventControl_data["course_" + iSeqRow]["change"]) {
    return true;
  }
  return false;
...
function scJQEventsAdd(iSeqRow) {
  $('#id_sc_field_id_' + iSeqRow).bind('change', function() { sc_sub_studentCourse_id__onchange(this, iSeqRow) });
  $('#id_sc_field_student_' + iSeqRow).bind('change', function() { sc_sub_studentCourse_student__onchange(this, iSeqRow) });
  $('#id_sc_field_course_' + iSeqRow).bind('blur', function() { sc_sub_studentCourse_course__onblur(this, iSeqRow) })
                                     .bind('change', function() { sc_sub_studentCourse_course__onchange(this, iSeqRow) })
                                     .bind('focus', function() { sc_sub_studentCourse_course__onfocus(this, iSeqRow) });
} // scJQEventsAdd

function sc_sub_studentCourse_id__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_sub_studentCourse_student__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
}

function sc_sub_studentCourse_course__onblur(oThis, iSeqRow) {
  do_ajax_sub_studentCourse_validate_course_(iSeqRow);
  scCssBlur(oThis, iSeqRow);
}

function sc_sub_studentCourse_course__onchange(oThis, iSeqRow) {
  scMarkFormAsChanged();
  nm_check_insert(iSeqRow);
}

function sc_sub_studentCourse_course__onfocus(oThis, iSeqRow) {
...
function displayChange_block_0(status) {
	displayChange_field("course_", "", status);
}

function displayChange_row(row, status) {
	displayChange_field_course_(row, status);
}
...
function displayChange_field_course_(row, status) {
    var fieldId;
}
...
function scHidePage(pageNo) {
	$("#id_sub_studentCourse_form" + pageNo).hide();
}
            ...
            if(!$(t).is('img')){
                ...
                switch(field){
                     ...
                     if(usa_read_only && $('a',$('#id_read_on_'+field+iSeqRow)).length == 0){
                         $(t).html("<a href=\"javascript:nm_mostra_doc('0', '"+rs2+"', 'sub_studentCourse')\">"+$('#id_read_on_'+field+iSeqRow).text()+"</a>");
                     }
                }
