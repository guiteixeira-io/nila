<?php
include_once('workspace_session.php');
...
if ($_SESSION['scriptcase']['device_mobile'])
{
    ...
    $_SESSION['scriptcase']['workspace']['glo_nm_path_prod']      = "/scriptcase/prod";
    $_SESSION['scriptcase']['workspace']['glo_nm_perfil']         = "";
    $_SESSION['scriptcase']['workspace']['glo_nm_conexao']        = "nila";
    $_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp'] = "/scriptcase/tmp";
    $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo']      = "S";
    ...
    //check prod
    if(empty($_SESSION['scriptcase']['workspace']['glo_nm_path_prod']))
    {
            /*check prod*/$_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] = $str_path_apl_url . "_lib/prod";
    }
    //check tmp
    if(empty($_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp']))
    {
            /*check tmp*/$_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp'] = $str_path_apl_url . "_lib/tmp";
    }
...
class workspace_class
{
 ...
 {
    global $workspace_menuData, $nm_data_fixa;
     ...
     { 
         $_SESSION['scriptcase']['str_schema_all'] = $_POST["nmgp_schema"] . "/" . $_POST["nmgp_schema"];
           ...
           $nm_versao_sc  = "" ; 
           $_SESSION['scriptcase']['workspace']['contr_erro'] = 'off';
           $Campos_Mens_erro = "";
...
$path_libs      = $str_root . $_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] . "/lib/php";
$path_third     = $str_root . $_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] . "/third";
$path_adodb     = $str_root . $_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] . "/third/adodb";
$path_apls      = $str_root . substr($path_link, 0, strrpos($path_link, '/'));
$path_img_old   = $str_root . $path_link . "workspace/img";
$this->path_css = $str_root . $path_link . "_lib/css/";
$_SESSION['scriptcase']['dir_temp'] = $str_root . $_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp'];
...
$this->sc_Include($path_libs . "/nm_ini_perfil.php", "F", "perfil_lib") ; 
 if(function_exists('set_php_timezone')) set_php_timezone('workspace');
...
if (!defined("SC_ERROR_HANDLER"))
{
    ...
    include_once(dirname(__FILE__) . "/workspace_erro.php");
}
include_once(dirname(__FILE__) . "/workspace_erro.class.php"); 
$this->Erro = new workspace_erro();
$str_path = substr($_SESSION['scriptcase']['workspace']['glo_nm_path_prod'], 0, strrpos($_SESSION['scriptcase']['workspace']['glo_nm_path_prod'], '/') + 1);
if (!is_file($str_root . $str_path . 'devel/class/xmlparser/nmXmlparserIniSys.class.php'))
{
    unset($_SESSION['scriptcase']['nm_sc_retorno']);
    unset($_SESSION['scriptcase']['workspace']['glo_nm_conexao']);
}

/* Definição dos Caminhos */
$workspace_menuData         = array();
$workspace_menuData['path'] = array();
$workspace_menuData['url']  = array();
$workspace_menuData['data'] = "";
$NM_dir_atual = getcwd();
if (empty($NM_dir_atual))
{
    $workspace_menuData['path']['sys'] = (isset($_SERVER['SCRIPT_FILENAME'])) ? $_SERVER['SCRIPT_FILENAME'] : $_SERVER['ORIG_PATH_TRANSLATED'];
    $workspace_menuData['path']['sys'] = str_replace("\\", '/', $str_path_sys);
    $workspace_menuData['path']['sys'] = str_replace('//', '/', $str_path_sys);
}
else
{
    $sc_nm_arquivo                                   = explode("/", $_SERVER['PHP_SELF']);
    $workspace_menuData['path']['sys'] = str_replace("\\", "/", str_replace("\\\\", "\\", getcwd())) . "/" . $sc_nm_arquivo[count($sc_nm_arquivo)-1];
}
$workspace_menuData['url']['web']   = $_SERVER['PHP_SELF'];
$workspace_menuData['url']['web']   = str_replace("\\", '/', $workspace_menuData['url']['web']);
$workspace_menuData['path']['root'] = substr($workspace_menuData['path']['sys'],  0, -1 * strlen($workspace_menuData['url']['web']));
$workspace_menuData['path']['app']  = substr($workspace_menuData['path']['sys'],  0, strrpos($workspace_menuData['path']['sys'],  '/'));
$workspace_menuData['path']['link'] = substr($workspace_menuData['path']['app'],  0, strrpos($workspace_menuData['path']['app'],  '/'));
$workspace_menuData['path']['link'] = substr($workspace_menuData['path']['link'], 0, strrpos($workspace_menuData['path']['link'], '/')) . '/';
$workspace_menuData['path']['app'] .= '/';
$workspace_menuData['url']['app']   = substr($workspace_menuData['url']['web'],  0, strrpos($workspace_menuData['url']['web'],  '/'));
$workspace_menuData['url']['link']  = substr($workspace_menuData['url']['app'],  0, strrpos($workspace_menuData['url']['app'],  '/'));
if ($_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] == "S")
{
    $workspace_menuData['url']['link']  = substr($workspace_menuData['url']['link'], 0, strrpos($workspace_menuData['url']['link'], '/'));
}
$workspace_menuData['url']['link']  .= '/';
$workspace_menuData['url']['app']   .= '/';


$_SESSION['scriptcase']['workspace']['sc_apl_link'] = $workspace_menuData['url']['link'];
...
if (isset($_SESSION['scriptcase']['workspace']['session_timeout']['lang'])) {
    $this->str_lang = $_SESSION['scriptcase']['workspace']['session_timeout']['lang'];
}
elseif (!isset($_SESSION['scriptcase']['workspace']['actual_lang']) || $_SESSION['scriptcase']['workspace']['actual_lang'] != $this->str_lang) {
    $_SESSION['scriptcase']['workspace']['actual_lang'] = $this->str_lang;
...
include_once("workspace_toolbar.php");
...
if ($_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] != "S")
{
    $this->tab_grupo[0] = "";
     ...
     $_SESSION['scriptcase']['menu_atual'] = "workspace";
     $_SESSION['scriptcase']['menu_apls']['workspace'] = array();
     if (isset($_SESSION['scriptcase']['sc_connection']) && !empty($_SESSION['scriptcase']['sc_connection']))
     {
         foreach ($_SESSION['scriptcase']['sc_connection'] as $NM_con_orig => $NM_con_dest)
         {
             if (isset($_SESSION['scriptcase']['workspace']['glo_nm_conexao']) && $_SESSION['scriptcase']['workspace']['glo_nm_conexao'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['workspace']['glo_nm_conexao'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['workspace']['glo_nm_perfil']) && $_SESSION['scriptcase']['workspace']['glo_nm_perfil'] == $NM_con_orig)
             {
/*NM*/           $_SESSION['scriptcase']['workspace']['glo_nm_perfil'] = $NM_con_dest;
             }
             if (isset($_SESSION['scriptcase']['workspace']['glo_con_' . $NM_con_orig]))
             {
                 $_SESSION['scriptcase']['workspace']['glo_con_' . $NM_con_orig] = $NM_con_dest;
             }
         }
     }
...
if (isset($_SESSION['scriptcase']['workspace']['session_timeout']['redir'])) {
...
';
    ...
    if ($_SESSION['scriptcase']['workspace']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "  </HEAD>\r\n";
        $SS_cod_html .= "   <body>\r\n";
    }
    else {
        ...
        $SS_cod_html .= "           <input type=\"button\" name=\"sc_sai_seg\" value=\"OK\" onclick=\"sc_session_redir('" . $_SESSION['scriptcase']['workspace']['session_timeout']['redir'] . "');\">\r\n";
        $SS_cod_html .= "     </form>\r\n";
    ...
    if ($_SESSION['scriptcase']['workspace']['session_timeout']['redir_tp'] == "R") {
        $SS_cod_html .= "      sc_session_redir('" . $_SESSION['scriptcase']['workspace']['session_timeout']['redir'] . "');\r\n";
    }
    ...
    $SS_cod_html .= " </body>\r\n";
    $SS_cod_html .= "</HTML>\r\n";
    unset($_SESSION['scriptcase']['workspace']['session_timeout']);
...
{
    $_SESSION['sc_session'][1]['workspace']['init'] = $script_case_init;
}
else
if (!isset($_SESSION['sc_session'][1]['workspace']['init']))
{
    $_SESSION['sc_session'][1]['workspace']['init'] = "";
}
$script_case_init = $_SESSION['sc_session'][1]['workspace']['init'];
...
if (isset($_SESSION['sc_session']['SC_parm_violation']) && !isset($_SESSION['scriptcase']['workspace']['session_timeout']['redir']))
{
...
{ 
    $nm_url_saida = $_SESSION['scriptcase']['sc_saida_workspace'];
}
elseif (!empty($nm_url_saida))
{
    $_SESSION['scriptcase']['sc_url_saida'][$script_case_init]  = $nm_url_saida;
    $_SESSION['scriptcase']['sc_saida_workspace'] = $nm_url_saida;
}
else
{
...
$_SESSION['nm_menu'] = array('prod' => $str_root . $_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] . '/third/COOLjsMenu/',
                              'url' => $_SESSION['scriptcase']['workspace']['glo_nm_path_prod'] . '/third/COOLjsMenu/');

if ((isset($nmgp_outra_jan) && $nmgp_outra_jan == "true") || (isset($_SESSION['scriptcase']['sc_outra_jan']) && $_SESSION['scriptcase']['sc_outra_jan'] == 'workspace'))
{
    $_SESSION['sc_session'][1]['workspace']['sc_outra_jan'] = true;
     unset($_SESSION['scriptcase']['sc_outra_jan']);
    $_SESSION['scriptcase']['sc_saida_workspace'] = "javascript:window.close()";
}
/* Variáveis de Configuração do Menu */
$workspace_menuData['iframe'] = TRUE;
...
/* Itens do Menu */
$_SESSION['scriptcase']['workspace']['contr_erro'] = 'on';
...
{
    $workspace_menuData['data'] .= "item_7|.|" . $nm_var_lab[0] . "|workspace_form_php.php?sc_item_menu=item_7&sc_apl_menu=dsb_studyspace&sc_apl_link=" . urlencode($workspace_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] . "|" . $nm_var_hint[0] . "|grp__NM__ico__NM__nila-logo30.png|" . $this->workspace_target('_self') . "|" . "\n";
}
else
{
...
$str_disabled = "N";
$str_link = "workspace_form_php.php?sc_item_menu=item_7&sc_apl_menu=dsb_studyspace&sc_apl_link=" . urlencode($workspace_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] . "";
if (!isset($_SESSION['scriptcase']['sc_apl_seg']['dsb_studyspace']) || strtolower($_SESSION['scriptcase']['sc_apl_seg']['dsb_studyspace']) != "on")
{
    $str_link = "#";
    $str_disabled = "Y";
    ...
    $workspace_menuData['data'][] = array(
        ...
        'target'   => " item-target=\"" . $this->workspace_target('_self') . "\"",
        'sc_id'    => "item_7",
...
if (isset($_SESSION['scriptcase']['sc_def_menu']['workspace']))
{
    $arr_menu_usu = $this->nm_arr_menu_recursiv($_SESSION['scriptcase']['sc_def_menu']['workspace']);
    $this->nm_gera_menus($str_menu_usu, $arr_menu_usu, 1, 'workspace');
    $workspace_menuData['data'] = $str_menu_usu;
}
if (is_file("workspace_help.txt"))
{
    $Arq_WebHelp = file("workspace_help.txt"); 
        ...
        foreach ($Tmp as $Cada_help)
        {
            ...
            if (!empty($Tmp1[0]) && isset($Tmp1[1]) && !empty($Tmp1[1]) && $Tmp1[0] == "menu" && is_file($str_root . $path_help . $Tmp1[1]))
            {
                ...
                $workspace_menuData['data'][] = array(
                    ...
                    'icon_aba' => $icon_aba,
                    'icon_aba_inactive' => $icon_aba_inactive,
                    'target'   => "" . $this->workspace_target('_blank') . "",
                    'sc_id'    => "item_Help",
                    'disabled' => $str_disabled,
                    'display'     => "text",
                    'display_position'=> "",
...
if (isset($_SESSION['scriptcase']['sc_menu_del']['workspace']) && !empty($_SESSION['scriptcase']['sc_menu_del']['workspace']))
{
    $nivel = 0;
    $exclui_menu = false;
    foreach ($workspace_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_del']['workspace']))
       {
          $nivel = $cada_menu['level'];
          $exclui_menu = true;
          unset($workspace_menuData['data'][$i_menu]);
       }
    ...
    $workspace_menuData['data'] = $Temp_menu;
}

if (isset($_SESSION['scriptcase']['sc_menu_disable']['workspace']) && !empty($_SESSION['scriptcase']['sc_menu_disable']['workspace']))
{
    $disable_menu = false;
    foreach ($workspace_menuData['data'] as $i_menu => $cada_menu)
    {
       if (in_array($cada_menu['id'], $_SESSION['scriptcase']['sc_menu_disable']['workspace']))
       {
          $nivel = $cada_menu['level'];
          $disable_menu = true;
          $workspace_menuData['data'][$i_menu]['disabled'] = 'Y';
       }
...
/* Cabeçalho HTML */
if ($workspace_menuData['iframe'])
{
    $workspace_menuData['height'] = '100%';
    header("X-XSS-Protection: 1; mode=block");
    header("X-Frame-Options: SAMEORIGIN");
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
...
<head>
 ...
 <link rel="stylesheet" href="<?php echo $_SESSION['scriptcase']['workspace']['glo_nm_path_prod']; ?>/third/font-awesome/css/all.min.css" type="text/css" media="screen" />
...
{
    $workspace_menuData['height'] = '30px';
}

/* Arquivos JS */
?>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['workspace']['glo_nm_path_prod']; ?>/third/jquery/js/jquery.js"></script>
<script type="text/javascript" src="../_lib/lib/js/menu_structure.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['workspace']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['workspace']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
<script type="text/javascript" src="workspace_message.js"></script>
...
{
  if (str_id != "iframe_workspace")
  {
      str_id        = str_id.replace("workspace_","");
  }
  ...
  if (typeof str_link !== typeof undefined && str_link !== false) {
    str_id = str_id.replace('iframe_workspace', 'workspace');
    //test link type
    ...
    if(str_target != '_blank' && $('#iframe_workspace').length)
        $('#iframe_workspace')[0].contentWindow.focus();
  }
}
</script>
...
<?php echo ($this->menu_orientacao=='vertical')?$this->nm_show_toolbarmenu($col_span, $saida_apl, $workspace_menuData, $path_imag_cab):''; ?>  <tr class="scMenuHTableCssAlt" id='idMenuLine'>
  <?php
  if($this->menu_orientacao != 'vertical')
  {
    ?>
...
echo $this->workspace_escreveMenu($workspace_menuData['data'], $path_imag_cab, $strAlign);
?></tr></table>
</div>
<?php
/* Controle de Iframe */
...
<?php echo $this->nm_show_toolbarmenu('', $saida_apl, $workspace_menuData, $path_imag_cab); ?><?php echo $this->nm_gera_degrade(1, $bg_line_degrade, $path_imag_cab); ?>  <tr>
<?php
}
else
{
...
{ 
    $SCR  = ($NM_scr_iframe != "" ? $NM_scr_iframe : "workspace_pag_ini.php");
} 
?>
      <iframe id="iframe_workspace" name="workspace_iframe" frameborder="0" class="scMenuIframe" style="width: 100%; height: 100%;"  src="<?php echo $SCR; ?>" <?php echo $link_default ?>></iframe>
<?php
}
?></div></td>
  </tr>
...
{
    echo "<script>";
    echo "   document.getElementById('iframe_workspace').click()";
    echo "</script>";
}
...
/* Controle de Target */
function workspace_escreveMenu($arr_menu, $path_imag_cab = '', $strAlign = '')
{
    global $nm_data_fixa;
    $last      = '';
    $itemClass = ' topfirst';
...
function workspace_target($str_target)
{
    global $workspace_menuData;
    ...
    elseif ($workspace_menuData['iframe'])
    {
        return 'workspace_iframe';
    }
    else
    {
        return $str_target;
...
function nm_show_toolbarmenu($col_span, $saida_apl, $workspace_menuData, $path_imag_cab)
{
    if(!empty($this->mobile_menu_toolbar) && ($this->force_mobile || ($_SESSION['scriptcase']['device_mobile'] && $_SESSION['scriptcase']['display_mobile'])))
    {
        return;
    ...
    <tr id="id_toolbar">
        <td class="scMenuHTableCss" align="right" style="margin:0px; padding:0px;" <?php echo $col_span; ?>>
            ...
            <?php
                ...
                {
                    echo nmButtonOutput($this->arr_buttons, "btn_10", "javascript:", "javascript:", "btn_10", "", "Ajustes", "", "absmiddle", "", "0px", $this->path_botoes, "", "Ajustes", "", "", "", "text_fontawesomeicon", "text_right", "1", "" . $this->workspace_target('_self') . "", "", "", "", "workspace", "");
                }

                if ($this->nmgp_botoes['btn_1'] == "on")
                {
                    echo nmButtonOutput($this->arr_buttons, "btn_1", "javascript:", "javascript:", "btn_1", "", "Notificaciones", "", "absmiddle", "", "0px", $this->path_botoes, "", "Notificaciones", "", "", "", "text_fontawesomeicon", "text_right", "1", "" . $this->workspace_target('_self') . "", "", "", "", "workspace", "");
                }

                if ($this->nmgp_botoes['btn_2'] == "on")
                {
                    echo nmButtonOutput($this->arr_buttons, "btn_2", "javascript:", "javascript:", "btn_2", "", "Perfil", "", "absmiddle", "", "0px", $this->path_botoes, "", "Perfil", "", "", "__sc_grp__", "text_fontawesomeicon", "text_right", "1", "" . $this->workspace_target('_self') . "", "", "", "", "workspace", "");
                    ?>
                            ...
                            <td class="scBtnGrpBackground">
                              ...
                              ?>
                                  <div class="scBtnGrpText">
                                    <?php
                                    $str_href = "workspace_form_php.php?sc_item_menu=btn_4&sc_apl_menu=cdt_user_loged&sc_apl_link=". urlencode($workspace_menuData['url']['link']) ."&sc_usa_grupo=". $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] ."";
                                    if(isset($this->arr_buttons['btn_4']['disabled']) && strtolower($this->arr_buttons['btn_4']['disabled']) == 'off')
                                    {
                                      $str_href = "javascript:";
                                    }
                                    ?>
                                      <a id="btn_4" href="<?php echo $str_href; ?>"   class="scBtnGrpLink" title="Tu" target="<?php echo $this->workspace_target('_self'); ?>" style="vertical-align: middle; display:inline-block;"><i class='icon_fa fas fa-user-circle'></i> Tu</a>
                                  </div>
                              <?php
                              }
                              ?>
                              ...
                              ?>
                                  <div class="scBtnGrpText">
                                    <?php
                                    $str_href = "workspace_form_php.php?sc_item_menu=btn_11&sc_apl_menu=sct_retrieve_pswd&sc_apl_link=". urlencode($workspace_menuData['url']['link']) ."&sc_usa_grupo=". $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] ."";
                                    if(isset($this->arr_buttons['btn_11']['disabled']) && strtolower($this->arr_buttons['btn_11']['disabled']) == 'off')
                                    {
                                      $str_href = "javascript:";
                                    }
                                    ?>
                                      <a id="btn_11" href="<?php echo $str_href; ?>"   class="scBtnGrpLink" title="Cambiar contraseña" target="<?php echo $this->workspace_target('_self'); ?>" style="vertical-align: middle; display:inline-block;"><i class='icon_fa fas fa-key'></i> Cambiar contraseña</a>
                                  </div>
                              <?php
                              }
                              ?>
                              ...
                              ?>
                                  <div class="scBtnGrpText">
                                    <?php
                                    $str_href = "workspace_form_php.php?sc_item_menu=btn_3&sc_apl_menu=sct_login&sc_apl_link=". urlencode($workspace_menuData['url']['link']) ."&sc_usa_grupo=". $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] ."";
                                    if(isset($this->arr_buttons['btn_3']['disabled']) && strtolower($this->arr_buttons['btn_3']['disabled']) == 'off')
                                    {
                                      $str_href = "javascript:";
                                    }
                                    ?>
                                      <a id="btn_3" href="<?php echo $str_href; ?>"   class="scBtnGrpLink" title="Salir" target="<?php echo $this->workspace_target('_parent'); ?>" style="vertical-align: middle; display:inline-block;"><i class='icon_fa fas fa-sign-out-alt'></i> Salir</a>
                                  </div>
                              <?php
                              }
                              ?>
   ...
   function nm_gera_menus(&$str_line_ret, $arr_menu_usu, $int_level, $nome_aplicacao)
   {
       ...
       foreach ($arr_menu_usu as $arr_item)
       {
           ...
           if ('' != $arr_item['link'])
           {
               ...
               {
                    $str_line['link'] = "workspace_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($workspace_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] . ""; 
               }
               else
               {
                    $str_line['link'] = "workspace_form_php.php?sc_item_menu=" . $arr_item['id'] . "&sc_apl_menu=" . $nome_apl . "&sc_apl_link=" . urlencode($workspace_menuData['url']['link']) . "&sc_usa_grupo=" . $_SESSION['scriptcase']['workspace']['glo_nm_usa_grupo'] . ""; 
           ...
           {
                $str_line['target'] = ('' != $arr_item['target'] && '' != $arr_item['link']) ?  $this->workspace_target( $arr_item['target']) : "_self"; 
           }
   ...
   {
        $_SESSION['scriptcase']['workspace']['sc_init'][$apl_menu] = 1;
        return  1;
   }
...
if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
    ...
    elseif (is_file($root . $_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp'] . "/sc_apl_default_nila.txt")) {
        $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['workspace']['glo_nm_path_imag_temp'] . "/sc_apl_default_nila.txt"));
    }
    if (isset($apl_def)) {
        if ($apl_def[0] != "workspace") {
            $_SESSION['scriptcase']['sem_session'] = true;
            if (strtolower(substr($apl_def[0], 0 , 7)) == "http://" || strtolower(substr($apl_def[0], 0 , 8)) == "https://" || substr($apl_def[0], 0 , 2) == "..") {
                $_SESSION['scriptcase']['workspace']['session_timeout']['redir'] = $apl_def[0];
            }
            else {
                $_SESSION['scriptcase']['workspace']['session_timeout']['redir'] = $path_aplicacao . "/" . SC_dir_app_name($apl_def[0]) . "/index.php";
            }
            $Redir_tp = (isset($apl_def[1])) ? trim(strtoupper($apl_def[1])) : "";
            $_SESSION['scriptcase']['workspace']['session_timeout']['redir_tp'] = $Redir_tp;
        }
        if (isset($_COOKIE['sc_actual_lang_nila'])) {
            $_SESSION['scriptcase']['workspace']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_nila'];
        }
    }
}
...
$contr_workspace = new workspace_class;
$contr_workspace->workspace_menu();
