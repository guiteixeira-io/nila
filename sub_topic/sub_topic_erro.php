...
$old_error_handler = set_error_handler("sub_topic_scriptcase_error_handler");

function sub_topic_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['sub_topic']['contr_erro']) && $_SESSION['scriptcase']['sub_topic']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sub_topic->contr_sub_topic->Apl_com_erro = true;
            if ($inicial_sub_topic->contr_sub_topic->NM_ajax_flag)
            {
                $inicial_sub_topic->contr_sub_topic->NM_ajax_info['errList']['geral_sub_topic'][] = $msg;
                return;
            }
            sub_topic_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sub_topic_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sub_topic'])  && '' != $_SESSION['scriptcase']['error_icon']['sub_topic'])  ? $_SESSION['scriptcase']['error_icon']['sub_topic']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sub_topic']) && '' != $_SESSION['scriptcase']['error_close']['sub_topic']) ? $_SESSION['scriptcase']['error_close']['sub_topic'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sub_topic']) && '' != $_SESSION['scriptcase']['error_span_title']['sub_topic']) ? $_SESSION['scriptcase']['error_span_title']['sub_topic'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sub_topic']) && '' != $_SESSION['scriptcase']['error_icon_title']['sub_topic']) ? $_SESSION['scriptcase']['error_icon_title']['sub_topic'] : '';
?>
