...
class mnu_course_old_form_php
{
   ...
   function init()
   {
      ...
      if(!isset($_SESSION['scriptcase']['mnu_course_old']['glo_nm_path_prod']) || empty($_SESSION['scriptcase']['mnu_course_old']['glo_nm_path_prod']))
      {
              /*check prod*/$_SESSION['scriptcase']['mnu_course_old']['glo_nm_path_prod'] = $str_path_apl_url . "_lib/prod";
      }
...
<?php
      ...
      if (isset($_GET['sc_apl_menu']))
      {
          ...
          $_SESSION['scriptcase']['sc_apl_menu_link'] = $_SESSION['scriptcase']['mnu_course_old']['sc_apl_link'];
...
if (!isset($this->sc_temp_curso)) {$this->sc_temp_curso = (isset($_SESSION['curso'])) ? $_SESSION['curso'] : "";}
 $this->sc_temp_curso = $this->sc_script_name;

if($this->sc_temp_curso!=NULL){	
 ...
 {
$this->nmgp_redireciona_form($_SESSION['scriptcase']['sc_apl_menu_link'] . $this->tab_grupo[0] . "" . SC_dir_app_name('mnu_subject') . "/", "mnu_course_old_form_php.php", "","_self", 440, 630);
 };
       }
...
$_SESSION['scriptcase']['mnu_course_old']['contr_erro'] = 'off';
      ...
      {
         foreach($_SESSION['scriptcase']['sc_def_menu']['mnu_course_old'] as $id_item => $arr_item)
         {
...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
      ...
      <html><body>
        <form name="fmenu" method="post" action="<?php echo NM_encode_input($apl_run); ?>">
          ...
          <input type=hidden name="nm_apl_menu" value="mnu_course_old"> 
<?php
      if (isset($_SESSION['scriptcase']['menu_mobile']) && $_SESSION['scriptcase']['menu_mobile'] == "mnu_course_old")
      {
?>
...
<?php
      ...
      {
?>
          window.history.pushState('Object', 'mnu_course_old', '<?php echo $this->nm_location ?>');
<?php
      }
      if ($link_url)
      {
...
<?php
   ...
   function Gera_sc_init($apl_menu)
   {
        ...
        $this->menu_sc_init = $_SESSION['scriptcase']['mnu_course_old']['sc_init'][$apl_menu];
   ...
   function nmgp_redireciona_form($nm_apl_dest, $nm_apl_retorno, $nm_apl_parms, $nm_target="", $alt_modal=0, $larg_modal=0)
   {
      if (is_array($nm_apl_parms))
      {
          ...
          foreach ($nm_apl_parms as $par => $val)
          {
              ...
              elseif (substr($val, 0, 1) == "[")
              {
                  $tmp_parms .= $_SESSION['sc_session'][1]['mnu_course_old'][substr($val, 1, -1)];
              }
              else
              {
                  $tmp_parms .= $val;
...
$controle = new mnu_course_old_form_php();
$controle->init();
?>
