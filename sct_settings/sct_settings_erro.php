...
$old_error_handler = set_error_handler("sct_settings_scriptcase_error_handler");

function sct_settings_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['sct_settings']['contr_erro']) && $_SESSION['scriptcase']['sct_settings']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sct_settings->contr_sct_settings->Apl_com_erro = true;
            if ($inicial_sct_settings->contr_sct_settings->NM_ajax_flag)
            {
                $inicial_sct_settings->contr_sct_settings->NM_ajax_info['errList']['geral_sct_settings'][] = $msg;
                return;
            }
            sct_settings_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sct_settings_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sct_settings'])  && '' != $_SESSION['scriptcase']['error_icon']['sct_settings'])  ? $_SESSION['scriptcase']['error_icon']['sct_settings']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sct_settings']) && '' != $_SESSION['scriptcase']['error_close']['sct_settings']) ? $_SESSION['scriptcase']['error_close']['sct_settings'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sct_settings']) && '' != $_SESSION['scriptcase']['error_span_title']['sct_settings']) ? $_SESSION['scriptcase']['error_span_title']['sct_settings'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sct_settings']) && '' != $_SESSION['scriptcase']['error_icon_title']['sct_settings']) ? $_SESSION['scriptcase']['error_icon_title']['sct_settings'] : '';
?>
