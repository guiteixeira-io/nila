...
$old_error_handler = set_error_handler("cdt_systemMessages_mob_scriptcase_error_handler");

function cdt_systemMessages_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['cdt_systemMessages_mob']['contr_erro']) && $_SESSION['scriptcase']['cdt_systemMessages_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_cdt_systemMessages_mob->contr_cdt_systemMessages_mob->Apl_com_erro = true;
            if ($inicial_cdt_systemMessages_mob->contr_cdt_systemMessages_mob->NM_ajax_flag)
            {
                $inicial_cdt_systemMessages_mob->contr_cdt_systemMessages_mob->NM_ajax_info['errList']['geral_cdt_systemMessages_mob'][] = $msg;
                return;
            }
            cdt_systemMessages_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function cdt_systemMessages_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['cdt_systemMessages_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['cdt_systemMessages_mob'])  ? $_SESSION['scriptcase']['error_icon']['cdt_systemMessages_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['cdt_systemMessages_mob']) && '' != $_SESSION['scriptcase']['error_close']['cdt_systemMessages_mob']) ? $_SESSION['scriptcase']['error_close']['cdt_systemMessages_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['cdt_systemMessages_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['cdt_systemMessages_mob']) ? $_SESSION['scriptcase']['error_span_title']['cdt_systemMessages_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['cdt_systemMessages_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['cdt_systemMessages_mob']) ? $_SESSION['scriptcase']['error_icon_title']['cdt_systemMessages_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['cdt_systemMessages']['error_buffer'] .= ob_get_flush();
}
