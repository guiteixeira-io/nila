...
$old_error_handler = set_error_handler("sct_form_edit_users_scriptcase_error_handler");

function sct_form_edit_users_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['sct_form_edit_users']['contr_erro']) && $_SESSION['scriptcase']['sct_form_edit_users']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sct_form_edit_users->contr_sct_form_edit_users->Apl_com_erro = true;
            if ($inicial_sct_form_edit_users->contr_sct_form_edit_users->NM_ajax_flag)
            {
                $inicial_sct_form_edit_users->contr_sct_form_edit_users->NM_ajax_info['errList']['geral_sct_form_edit_users'][] = $msg;
                return;
            }
            sct_form_edit_users_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sct_form_edit_users_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sct_form_edit_users'])  && '' != $_SESSION['scriptcase']['error_icon']['sct_form_edit_users'])  ? $_SESSION['scriptcase']['error_icon']['sct_form_edit_users']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sct_form_edit_users']) && '' != $_SESSION['scriptcase']['error_close']['sct_form_edit_users']) ? $_SESSION['scriptcase']['error_close']['sct_form_edit_users'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sct_form_edit_users']) && '' != $_SESSION['scriptcase']['error_span_title']['sct_form_edit_users']) ? $_SESSION['scriptcase']['error_span_title']['sct_form_edit_users'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sct_form_edit_users']) && '' != $_SESSION['scriptcase']['error_icon_title']['sct_form_edit_users']) ? $_SESSION['scriptcase']['error_icon_title']['sct_form_edit_users'] : '';
?>
